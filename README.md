...
Примерная структура каталога проекта:
...
Для демонстрации работы сценариев инцидентов и статистических модулей может использоваться специальный тестовый файл
журнала, содержащий:

* серию неуспешных попыток SSH-логина;
* события изменения файлов `/etc/shadow` и `/etc/ssh/sshd_config`;
* запуск `/bin/bash` от имени сервисного пользователя.

Рекомендации по использованию:

1. В корневой папке проекта содержится небольшой тестовый файл `test_log.log`.
